...
void dice::bet(const name player, const asset bet_amount, const uint32_t roll_under)
{
   require_auth(player);
...
void dice::resolvebet(const uint64_t bet_id)
{
   require_auth(_self);
   ...
   if (ref_reward > zero_poc)
   {
      // INLINE_ACTION_SENDER(potato::token, transfer)(
      //    token_account, { {_self, active_permission} },
      //    { _self, dice_edge, asset(ref_reward, token_symbol), "unallocated inflation" }
      // );
      transfer.actions.emplace_back(
          potato::permission_level{_self, active_permission},
          token_account,
...
void dice::refundbet(const name player, const uint64_t bet_id)
{
   require_auth(player);
   auto it = bet_stats.find(bet_id);
...
uint64_t dice::get_token_balance(const name account, const symbol_code &token_type) const
{
   auto balance = potato::token::get_balance("pc.token"_n, account, token_type);
   return (uint64_t)balance.amount;
}

uint64_t dice::get_payout_mult_times10000(const uint32_t roll_under, const uint64_t house_edge_times_10000) const
{
   return ((10000 - house_edge_times_10000) * 100) / (roll_under - 1);
}

uint64_t dice::get_max_win() const
{
...
void dice::test2(const name player)
{
   require_auth(_self);
   my_clearing_stats player_bet_clearing_stats(_self, player.value);
...
POTATO_DISPATCH(potato::dice, (bet)(resolvebet)(refundbet)(test)(test2))
